...
#      @balls = [] unless @balls.empty?
  ...
  def configure_level(current_config)
    ...
    #set up level balls
    @balls = []
    @balls = @balls + current_config[:from_top].times.map {@ball_class.new(@game_window, @player, 0, current_config[:yinc] ? current_config[:yinc] : 10, lambda {rand(@game_window.width-100)}, lambda {0})} if current_config[:from_top]
    ...
    @balls = @balls + current_config[:from_bottom].times.map {@ball_class.new(@game_window, @player, 0, current_config[:yinc] ? current_config[:yinc] : -10, lambda {rand(@game_window.width-100)}, lambda {@game_window.height})} if current_config[:from_bottom]
